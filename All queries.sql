...
SELECT Entity, Code, GDP_constant_2015_US 
FROM GDP 
WHERE GDP_constant_2015_US > 
	(
     SELECT GDP_constant_2015_US 
	 FROM GDP 
     WHERE Entity = "European Union"
	) AND Code LIKE "___"; 
...
FROM country
INNER JOIN (
			SELECT CountryCode, SUM(city.population) AS CityPopulation 
...
CREATE OR REPLACE VIEW GDPview AS
	   SELECT Code,
			  Entity,
			  Rank()OVER (ORDER BY (GDP_constant_2015_US) DESC) AS RankbyGDP,
			  GDP_constant_2015_US AS GDP
	   FROM world.GDP
	   WHERE `Code` !=''
	   -- and deleting the "World" and "Kosovo" rows from the view
	   AND
		  `Entity` NOT IN
		  (
		   SELECT Entity
...
-- Creates a view that compares the Health Expenditure to Life Expectancy.
CREATE OR REPLACE VIEW HE_LE AS
SELECT Country.Code AS cCode,
	   Country.Name AS cName,
       HE.`HE_PER_CAPITA(USD)` AS `HE /Capita`,
       Country.LifeExpectancy AS LE
FROM HE_Per_Capita AS HE
INNER JOIN Country
ON HE.Country_Code = Country.Code;

SELECT * 
FROM world.GDPview;
SELECT * 
FROM world.OlympicsView;
SELECT * 
FROM world.GDP_Olympics;
SELECT *
FROM HE_LE;